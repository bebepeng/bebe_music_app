...
describe SongList do
  ...
  it "adds a song to the database" do
    ...
    expect(@songlist.all).to eq [{:id => id, :artist => "some_artist", :title => "some_title", :language => "some_language"}]
  end

  it "edits a song's attributes" do
    id = @songlist.add("some_artist", "some_title", "some_language")
    @songlist.edit(id, {:artist => "other_artist"})
    expect(@songlist.all).to eq [{:id => id, :artist => "other_artist", :title => "some_title", :language => "some_language"}]
  end
